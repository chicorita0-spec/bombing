...
const CONFIG_DEFAULT = { bossName:'피즐리베어', maxHp:33000, minDamage:1, maxDamage:1500, multiCount:11, shotInterval:240, apiEnabled:false, guideEnabled:true };
...
async function animateShot(result,attacker,{finalShot=false}={}){
  ...
  setSprite(bombing,SPRITES.bombingAttack);await wait(160);if(dead)return;setSprite(bombing,SPRITES.bombingShoot);restartAnimation(bombing,'fire');restartAnimation(muzzle,'on');gunSound();muzzleBurst(startX,startY);screenShake(false);
  const bullet=document.createElement('div');bullet.className=`bullet${isMiss?' miss-bullet':''}`;bullet.style.left=`${startX}px`;bullet.style.top=`${startY}px`;bullet.style.width=`${Math.max(90,(hitX-startX)*.2)}px`;bullet.style.setProperty('--travel',`${hitX-startX}px`);bullet.style.setProperty('--rise',`${hitY-startY}px`);fxLayer.appendChild(bullet);await wait(110);if(!dead)setSprite(bombing,SPRITES.bombingIdle);bullet.remove();if(dead)return;
